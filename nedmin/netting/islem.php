...
//eger karsıdaki kişi bizim sqlkodlarımıza sızmaya calısır vs olursa dıye html kodlarını temızler
...
// Başlangıç mukerrer kayıt varmı yok mu yanio kişi bende daha önceden kayıtlımı ??
			...
			if ($say==0) {// yani dönmemiş kullanıcı yok
...
if (isset($_POST['kullaniciduzenle'])) {
	...
	echo $kullanici_id=$_POST['kullanici_id'];
		...
		kullanici_adres=:kullanici adres/il/ilce/unvan/postakodu/ulkevs*/
...
if (isset($_POST['logoduzenle'])) {
	...
	if ($_FILES['ayar_logo']['size']>1048576) {
		echo "bu dosya boyutu çok buyuk ";
...
/*if (isset($_POST['urunfotosira'])) {
 $urun_id=$_POST['urun_id'];
 
 $kaydet=$db->prepare("UPDATE urunfoto SET
 ...
 if ($update) {
 Header("Location:../production/urun-galeri.php?urun_id=$urun_id&durum=yes");
 } else {
 Header("Location:../production/urun-galeri.php?urun_id=$urun_id&durum=no");
 }
}*/
...
if(isset($_POST['urunsepetsil'])) {

	$sepet_id=$_POST['sepet_id'];


	echo $checklist = $_POST['urunsec'];
	...
	if ($kontrol) {
		Header("Location:../../sepet.php?sepet_id=$sepet_id&durum=ok");

	} else {

		Header("Location:../../sepet.php?sepet_id=$sepet_id&durum=no");
	}
